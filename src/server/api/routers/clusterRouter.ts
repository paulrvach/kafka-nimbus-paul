...
export const clusterRouter = createTRPCRouter({
  createCluster: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        const userSession = ctx.session?.user.id;
        if (!userSession)
          throw Error("No user session in clusterRouter.createCluster");
        const userResponse = await ctx.db.query.users.findFirst({
          where: eq(users.id, userSession),
        });

        if (!userResponse?.vpcId) {
          throw new Error("User doesn't exist in database");
        }
        // store the vpcId and subnets l
        const vpcId = userResponse.vpcId;
        const subnetIds: string[] = JSON.parse(
          userResponse.subnetId as string,
        ) as string[];
        ...
        const createSecurityGroupData = await awsService.createSecurityGroup(
          ec2,
          vpcId,
        );

        await awsService.authorizeSecurityGroupIngress(
          ec2,
        ...
        // kafka params
        const kafkaParams: AWS.Kafka.CreateClusterRequest = {
          BrokerNodeGroupInfo: {
        ...
        const kafkaData = await awsService.createKafkaCluster(
          kafka,
          kafkaParams,
        );
        ...
        const response = await ctx.db.insert(clusters).values({
          cluster_id: Math.random().toLocaleString(),
          name: input.name,
          bootStrapServer: input.brokerPerZone,
          securityGroup: JSON.stringify([createSecurityGroupData]),
  ...
  checkClusterStatus: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!userId)
        throw Error("No User Session in clusterRouter.checkClusterStatus");
      ...
      try {
        const {
          ...
          lifeCycleStage,
          kafkaArn,
        } = await checkService.getClusterResponse(input.id, userId);
        ...
        if (
          ...
          throw Error("ERROR checkService.getClusterResponse IN CLUSTERROUTER");
        ...
        const kafka = new KafkaClient({apiVersion: "2018-11-14"});
        ...
        if (kafkaArn) {
          ...
          if (curState === "ACTIVE" && lifeCycleStage === 0) {
            await checkService.updatePublicAccess(
              region,
              awsAccessKey,
              awsSecretAccessKey,
              kafkaArn,
          ...
          // grabs boostrap broker strings and stores them in the db
            ...
            await checkService.getBoostrapBrokers(
              region,
              awsAccessKey,
              awsSecretAccessKey,
              kafkaArn,
  ...
  deleteCluster: publicProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        const toDelete = await ctx.db.query.clusters.findFirst({
          where: eq(clusters.cluster_id, input.id),
        });
        ...
        const accessKeyId = userResponse.awsAccessKey;
        const secretAccessKey = userResponse.awsSecretAccessKey;
        const region = userResponse.region;
        const ClusterArn = userResponse.configArn ? userResponse.configArn : "";

        if (!region || !accessKeyId || !secretAccessKey)
          throw Error(
            "Error in clusterRouter.deleteCluster unsuccessful database query",
          );
        ...
        if (response.State !== "DELETING")
          ...
          const deletedCluster = await ctx.db
            .delete(clusters)
            .where(eq(clusters.cluster_id, input.id));
          if (!deletedCluster)
   ...
   */
  updateTargetsJson: publicProcedure.query(() => {
  ...
  getClusters: publicProcedure.query(async ({ ctx }) => {
    try {
      const userId = ctx.session?.user.id;
      if (!userId) throw Error("no user id");
      const usersClusters = await ctx.db.query.clusters.findMany({
        where: eq(clusters.user_id, userId),
      });

      return usersClusters;
    } catch (err) {
      console.log("Error in clusterRouter.getCluster", err);
    }
  }),
});
