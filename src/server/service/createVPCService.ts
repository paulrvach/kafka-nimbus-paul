...
export const createVPC = async (client: EC2Client) => {
  ...
  try {
    ...
    if (response?.Vpc?.VpcId === undefined) {
...
export const createIGW = async (client: EC2Client) => {
  try {
    ...
    const response = await client.send(command);
    if (response?.InternetGateway?.InternetGatewayId === undefined) {
...
export const connectIGWandVPC = async (
  client: EC2Client,
  vpcId: string,
  igwId: string,
) => {
...
export const createRouteTables = async (client: EC2Client, vpcId: string) => {
  try {
    ...
    if (
      response?.RouteTables === undefined ||
      response.RouteTables?.length === 0
    ) {
...
export const createRouteIGW = async (
  client: EC2Client,
  routeTableId: string,
  igwId: string,
) => {
...
export const createClusterConfig = async (client: KafkaClient) => {
  try {
    ...
    const ServerProperties = fs.readFileSync(
      "src/server/api/routers/server.properties",
    );
    const configParams = {
